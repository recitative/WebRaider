...
public class HttpRequestHandler {
    ...
    public static void sendRequest(String targetUrl, String requestMethod, String postData) throws IOException {
        ...
        try {
            ...
            System.out.printf(ConsoleColors.GREEN + "[INFO]" + ConsoleColors.RESET + " [%s] Resp: %d%n", timestamp, responseCode);
    ...
    private static void retryRequest(HttpURLConnection connection) throws IOException {
        ...
        while (responseCode != 200 && retries > 0) {
            System.out.printf(ConsoleColors.YELLOW + "[WARN]" + ConsoleColors.RESET + " Retry %d for response %d%n", 4 - retries, responseCode);
            retries--;
            connection.connect();
